...
#include "is_supported_platform.h"
  ...
  darktable.iop_priorities = dt_ioppr_get_iop_priorities();
  darktable.iop_priority_rules = dt_ioppr_get_iop_priority_rules();
  // load the darkroom mode plugins once:
  dt_iop_load_modules_so();
  // check if all modules have a priority assigned
  if(dt_ioppr_check_so_iop_priorities(darktable.iop, darktable.iop_priorities)) return 1;
