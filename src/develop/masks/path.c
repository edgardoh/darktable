...
static int _path_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, const float iop_order,
...
static int dt_path_get_points_border(dt_develop_t *dev, dt_masks_form_t *form, float **points,
                                     int *points_count, float **border, int *border_count, int source)
{
  return _path_get_points_border(dev, form, 0.f, DT_DEV_TRANSFORM_DIR_ALL, dev->preview_pipe, points, points_count,
                                   ...
                                   dt_masks_form_t *form, int *width, int *height, int *posx, int *posy)
{
  ...
  if(!_path_get_points_border(module->dev, form, module->iop_order, DT_DEV_TRANSFORM_DIR_BACK_INCL, piece->pipe,
                              &points, &points_count, &border, &border_count, 1))
  {
    free(points);
    free(border);
    return 0;
                            ...
                            int *width, int *height, int *posx, int *posy)
{
  ...
  int points_count, border_count;
  if(!_path_get_points_border(module->dev, form, module->iop_order, DT_DEV_TRANSFORM_DIR_BACK_INCL, piece->pipe,
                              &points, &points_count, &border, &border_count, 0))
  {
    free(points);
    free(border);
    return 0;
