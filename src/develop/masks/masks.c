                                     ...
                                     dt_masks_form_gui_t *gui)
{
  ...
  dev_add_masks_history_item(dev, module, TRUE);
...
int dt_masks_form_duplicate(dt_develop_t *dev, int formid)
{
  ...
  // we save the form
  dev_add_masks_history_item(dev, NULL, TRUE);
...
void dt_masks_form_remove(struct dt_iop_module_t *module, dt_masks_form_t *grp, dt_masks_form_t *form)
{
  ...
  if(!(form->type & (DT_MASKS_CLONE|DT_MASKS_NON_CLONE)) && grp)
  {
    ...
    if(ok) dev_add_masks_history_item(darktable.develop, module, TRUE);
    if(ok && module)
    {
      dt_masks_iop_update(module);
  ...
  if(form_removed) dev_add_masks_history_item(darktable.develop, module, TRUE);
}
...
void dt_masks_form_change_opacity(dt_masks_form_t *form, int parentid, int up)
{
  ...
  while(fpts)
  {
    ...
    if(fpt->formid == id)
    {
      ...
      {
        fpt->opacity = nv;
        dev_add_masks_history_item(darktable.develop, NULL, TRUE);
        dt_masks_update_image(darktable.develop);
      }
      break;
    }
...
void dt_masks_cleanup_unused(dt_develop_t *dev)
{
  ...
  // to remove it from dev->forms we have to check the next masks snapshot
  // after history_end, or the entire history if there are no snapshot
  int num = dev->history_end+1;
  int history_end = g_list_length(dev->history);
  GList *history = g_list_nth(dev->history, num);
  while(history)
  {
    dt_dev_history_item_t *hist = (dt_dev_history_item_t *)history->data;
    if(hist->forms)
  ...
  _masks_cleanup_unused(&dev->forms, dev->history, history_end);
