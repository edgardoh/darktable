...
static void image_srgb_to_rgb_bw(float *img_src, const int width, const int height, const int ch, float *img_dest, const float beta)
{
...
static void image_srgb_to_lab_bw(float *img_src, const int width, const int height, const int ch, float *img_dest)
{
  const int stride = width * height;
...
static void pad_image(float *img_src, const int width, const int height, const int ch, float *img_dest, 
    const int pad_w, const int pad_h, const int ch_padded, 
    ...
    const dt_iop_colorspace_type_t cst, const float beta)
{
  ...
  memset(img_dest, 0, iwidth * iheight * ch_padded * sizeof(float));
...
#pragma omp parallel for default(none) shared(img_src, img_dest) schedule(static) reduction(min : min) reduction(max : max)
    ...
    for (int y = 0; y < height; y++)
    {
      ...
      float *d = img_dest + (y + pad_h) * iwidth * ch_padded + pad_w * ch_padded;
      
      for (int x = 0; x < width; x++)
      {
        for (int c = 0; c < ch_padded; c++)
        {
          d[x*ch_padded+c] = RGBtosRGB(s[x*ch+c], 1.f);
        }
        
        const float l = ( rgb_luminance(d[x*ch_padded+0], d[x*ch_padded+1], d[x*ch_padded+2]) );
        min = MIN(min, l);
        max = MAX(max, l);
      }
    }
...
#pragma omp parallel for default(none) shared(img_src, img_dest) schedule(static) reduction(min : min) reduction(max : max)
    ...
    for (int y = 0; y < height; y++)
    {
      ...
      for (int x = 0; x < width; x++)
      {
        ...
        dt_XYZ_to_sRGB(XYZ, d + x*ch_padded);
        
        const float l = d[x*ch_padded];
        min = MIN(min, l);
        max = MAX(max, l);
      }
    }
...
if(0)  symetrize_image(img_dest, img_dest, iwidth, iheight, ch_padded, pad_w, pad_h, beta);
...
static void gradient_rgb(float *img_src, float *img_dest_x, float *img_dest_y, float *img_dest_n, const int width, const int height, const int ch,
    const int pad_w, const int pad_h, const int ch_padded,
  ...
  for (int y = 1; y < height-1; y++)
  {
    ...
    for (int x = 1; x < width-1; x++)
    {
      ...
      for (int c = 0; c < ch; c++)
      {
        ...
        RGBn[c] = sqrtf(RGBx[c]*RGBx[c] + RGBy[c]*RGBy[c]);
      }
...
//      if (x >= pad_w-1 && x < width-pad_w-1 && y >= pad_h-1 && y < height-pad_h-1)
        n = 1e-15 + powf(RGBn[max_bn], oddness);
//      else
//        n = 1.f;
...
static void estimate_laplacian(float *img_src_x, float *img_src_y, float *img_grn, float *img_dest, const int width, const int height, 
    ...
    const float _beta, const float image_scale, const float oddness)
{
  ...
  float *f = NULL;
  float *f_bp = NULL;
  float *boundary_image = NULL;

  f = dt_alloc_align(64, width * height * sizeof(float));
  if (f == NULL) goto cleanup;
  
  f_bp = dt_alloc_align(64, width * height * sizeof(float));
  if (f_bp == NULL) goto cleanup;

  boundary_image = dt_alloc_align(64, width * height * sizeof(float));
  if (boundary_image == NULL) goto cleanup;

  memset(img_dest, 0, stride * sizeof(float));
  memset(f, 0, stride * sizeof(float));
  memset(f_bp, 0, stride * sizeof(float));
  memcpy(boundary_image, img_grn, stride * sizeof(float));
...
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(img_src_x, img_src_y, f) schedule(static)
#endif
  for (int y = 0; y < height; y++)
  {
...
*/
  ...
  {
    float *d = f_bp + y * width;
    float *bi0 = boundary_image + y * width;
    float *bi1 = boundary_image + (y + 1) * width;
    float *bi_1 = boundary_image + (y - 1) * width;
...
//  normalize(img_grn, width, height, -1.f, 1.f);
  
  for (int y = 0; y < height; y++)
  {
    float *d = img_dest + y * width;
    float *f1 = f + y * width;
    float *bi = f_bp + y * width;
//    float *grn = img_grn + y * width;

    for (int x = 0; x < width; x++)
    {
      d[x] = (f1[x] - bi[x]);
//      d[x] *= powf(image_scale, 1.f-grn[x]);
//      d[x] *= powf(_beta, 1e-15 + powf(grn[x], oddness));
    }
  }

/*
  normalize(img_grn, width, height, 0.f, 1.f);
  
#define beta(x, y) (image_scale * grn[x])
...
//  const int pad_h = _pad_h;
  ...
  */
  ...
  if (boundary_image) dt_free_align(boundary_image);
...
//  const int ch_padded = 3;
  const int ch_padded = 1;
  ...
  img_padded = dt_alloc_align(64, iwidth * iheight * ch_padded * sizeof(float));
  if (img_padded == NULL) goto cleanup;
  
  pad_image(img_src, width, height, ch, img_padded, pad_w, pad_h, ch_padded, &l_min, &l_max, cst, beta);
  
  gradient_rgb(img_padded, img_grx, img_gry, img_grn, iwidth, iheight, ch_padded, pad_w, pad_h, ch_padded, oddness, image_scale, beta);
  ...
  if (cst == iop_cs_rgb)
    image_srgb_to_rgb_bw(img_unpad_gr, width, height, ch, img_dest, beta);
  else
    image_srgb_to_lab_bw(img_unpad_gr, width, height, ch, img_dest);
