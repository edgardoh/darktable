                  ...
                  void *new_params, const int new_version)
{
  if(old_version == 1 && new_version == 3)
  {
    ...
    *n = *d; // start with a fresh copy of default parameters
    ...
    {
      n->rt_forms[i].formid = o->rt_forms[i].formid;
      n->rt_forms[i].scale = o->rt_forms[i].scale;
      n->rt_forms[i].algorithm = o->rt_forms[i].algorithm;
      
//      n->rt_forms[i].blur_type = o->rt_forms[i].blur_type;
      n->rt_forms[i].blur_radius = o->rt_forms[i].blur_radius;
      
      n->rt_forms[i].fill_mode = o->rt_forms[i].fill_mode;
      n->rt_forms[i].fill_color[0] = o->rt_forms[i].fill_color[0];
      n->rt_forms[i].fill_color[1] = o->rt_forms[i].fill_color[1];
      n->rt_forms[i].fill_color[2] = o->rt_forms[i].fill_color[2];
//      n->rt_forms[i].fill_brightness = o->rt_forms[i].fill_brightness;
    }
...
//    n->fill_brightness = o->fill_brightness;
  ...
  else if(old_version == 3 && new_version == 3)
  {
    dt_iop_retouch_params_t *o = (dt_iop_retouch_params_t *)old_params;
    dt_iop_retouch_params_t *n = (dt_iop_retouch_params_t *)new_params;
...
static void _retouch_cairo_paint_tool_clone(cairo_t *cr,
                                          const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
static void _retouch_cairo_paint_paste_forms(cairo_t *cr, const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
static void _retouch_cairo_paint_cut_forms(cairo_t *cr, const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
static void _retouch_cairo_paint_show_final_image(cairo_t *cr, const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
static void _retouch_cairo_paint_show_current_scale(cairo_t *cr, const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
static void _retouch_cairo_paint_keep_current_scale(cairo_t *cr, const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
static void _retouch_cairo_paint_auto_levels(cairo_t *cr, const gint x, const gint y, const gint w, const gint h, const gint flags, void *data)
{
  PREAMBLE;
...
/*  if (p->preview_type == dt_iop_retouch_preview_current_scale || p->preview_type == dt_iop_retouch_preview_keep_current_scale)
...
static void rt_show_forms_for_current_scale(dt_iop_module_t *self)
{
  ...
  if (bd->masks_shown == DT_MASKS_EDIT_OFF || count == 0)
  {
    ...
    if(g) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(g->bt_edit_masks), 
...
static gboolean rt_wdbar_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
  ...
  if (p->curr_scale >= 0 && p->curr_scale < RETOUCH_NO_SCALES)
  {
    ...
    cairo_rectangle(cr,
        box_w * p->curr_scale + inset + DT_PIXEL_APPLY_DPI(3),
        inset + arrw * .5f + DT_PIXEL_APPLY_DPI(1),
        box_w - 2 * DT_PIXEL_APPLY_DPI(3),
  ...
  if (g->wdbar_mouse_y > arrw * .5f && g->wdbar_mouse_y < height - arrw * .5f)
  {
    ...
    if (curr_scale >= 0 && curr_scale < RETOUCH_NO_SCALES)
    {
      ...
      cairo_rectangle(cr,
          box_w * curr_scale + inset + DT_PIXEL_APPLY_DPI(1),
          inset + DT_PIXEL_APPLY_DPI(1),
          box_w - 2 * DT_PIXEL_APPLY_DPI(1),
          box_h - 2 * DT_PIXEL_APPLY_DPI(1));
      cairo_stroke(cr);
    }
  }
...
static gboolean rt_levelsbar_scrolled(GtkWidget *widget, GdkEventScroll *event, dt_iop_module_t *self)
{
  ...
  if(dt_gui_get_scroll_unit_deltas(event, NULL, &delta_y))
  {
    ...
    {
      float levels[3] = { p->preview_levels[0], p->preview_levels[1], p->preview_levels[2] };
      levels[lvl_idx] = CLAMP(levels[lvl_idx] - (0.05 * delta_y), RETOUCH_PREVIEW_LVL_MIN, RETOUCH_PREVIEW_LVL_MAX);
      rt_preview_levels_update(levels, self);
    }
  }
...
static gboolean rt_levelsbar_draw(GtkWidget *widget, cairo_t *crf, dt_iop_module_t *self)
{
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    const gboolean is_under_mouse = g->lvlbar_mouse_x >= 0.f && 
        rt_mouse_x_to_levels_index(g->lvlbar_mouse_x, p->preview_levels, width) == i;
...
void gui_init(dt_iop_module_t *self)
{
  ...
  g->bt_brush = dtgtk_togglebutton_new(dtgtk_cairo_paint_masks_brush, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_ellipse = dtgtk_togglebutton_new(dtgtk_cairo_paint_masks_ellipse, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_circle = dtgtk_togglebutton_new(dtgtk_cairo_paint_masks_circle, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_edit_masks = dtgtk_togglebutton_new(dtgtk_cairo_paint_masks_eye, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_fill = dtgtk_togglebutton_new(_retouch_cairo_paint_tool_fill, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_blur = dtgtk_togglebutton_new(_retouch_cairo_paint_tool_blur, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_heal = dtgtk_togglebutton_new(_retouch_cairo_paint_tool_heal, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_clone = dtgtk_togglebutton_new(_retouch_cairo_paint_tool_clone, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_showmask = dtgtk_togglebutton_new(dtgtk_cairo_paint_showmask, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_suppress = dtgtk_togglebutton_new(dtgtk_cairo_paint_eye_toggle, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_show_final_image = dtgtk_togglebutton_new(_retouch_cairo_paint_show_final_image, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_show_current_scale = dtgtk_togglebutton_new(_retouch_cairo_paint_show_current_scale, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_keep_current_scale = dtgtk_togglebutton_new(_retouch_cairo_paint_keep_current_scale, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  // copy/paste shapes
  g->bt_copy_scale = dtgtk_togglebutton_new(_retouch_cairo_paint_cut_forms, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->bt_paste_scale = dtgtk_togglebutton_new(_retouch_cairo_paint_paste_forms, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g_signal_connect(G_OBJECT(g->preview_levels_bar), "motion-notify-event", G_CALLBACK(rt_levelsbar_motion_notify), self);
  g_signal_connect(G_OBJECT(g->preview_levels_bar), "leave-notify-event", G_CALLBACK(rt_levelsbar_leave_notify), self);
  g_signal_connect(G_OBJECT(g->preview_levels_bar), "button-press-event", G_CALLBACK(rt_levelsbar_button_press), self);
  ...
  g->bt_auto_levels = dtgtk_togglebutton_new(_retouch_cairo_paint_auto_levels, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL);
  ...
  g->color_picker = GTK_TOGGLE_BUTTON(dtgtk_togglebutton_new(dtgtk_cairo_paint_colorpicker, CPF_STYLE_FLAT | CPF_DO_NOT_USE_BORDER, NULL));
  ...
  g_object_set(g->sl_fill_brightness, "tooltip-text", _("adjsts color brightness to fine tune it. works with erase as well."), (char *)NULL);
...
static void rt_compute_roi_in(struct dt_iop_module_t *self, struct dt_dev_pixelpipe_iop_t *piece, dt_iop_roi_t *roi_in, 
    int *_roir, int *_roib, int *_roix, int *_roiy)
{
...
static void rt_extend_roi_in_from_source_clones(struct dt_iop_module_t *self, struct dt_dev_pixelpipe_iop_t *piece, dt_iop_roi_t *roi_in, 
    const int formid_src, const int fl_src, const int ft_src, const int fw_src, const int fh_src, 
    int *_roir, int *_roib, int *_roix, int *_roiy)
{
  dt_iop_retouch_params_t *p = (dt_iop_retouch_params_t *)piece->data;
  dt_develop_blend_params_t *bp = self->blend_params;
...
static void rt_extend_roi_in_for_clone(struct dt_iop_module_t *self, struct dt_dev_pixelpipe_iop_t *piece, dt_iop_roi_t *roi_in, 
    int *_roir, int *_roib, int *_roix, int *_roiy)
{
  dt_iop_retouch_params_t *p = (dt_iop_retouch_params_t *)piece->data;
  dt_develop_blend_params_t *bp = self->blend_params;
...
//        dt_masks_form_t *form = dt_masks_get_from_id(self->dev, formid);
        ...
        const int intersects = !(roib < ft_src || ft_src + fh_src < roiy || 
                                 roir < fl_src || fl_src + fw_src < roix);
        if (intersects)
          rt_extend_roi_in_from_source_clones(self, piece, roi_in, formid, fl_src, ft_src, fw_src, fh_src, &roir, &roib, &roix, &roiy);
      }
...
#ifdef _OPENMP
#pragma omp parallel for default(none) schedule(static) reduction(+ : count, l_sum) reduction(max : l_max) reduction(min : l_min)
#endif
...
#pragma omp parallel for default(none) schedule(static)
  ...
  {
    const int mask_index = yy * roi_mask_scaled->width;
    const int dest_index = (((yy + roi_mask_scaled->y - roi_img->y) * roi_img->width) + (roi_mask_scaled->x - roi_img->x)) * ch;
...
static void retouch_fill(float *const in, dt_iop_roi_t *const roi_in, const int ch, 
                          float *const mask_scaled, dt_iop_roi_t *const roi_mask_scaled, 
                          const float opacity, 
                          const float *const fill_color, const int use_sse)
{
#if defined(__SSE__)
  if (ch == 4 && use_sse)
  {
...
static void retouch_clone(float *const in, dt_iop_roi_t *const roi_in, const int ch, 
                          float *const mask_scaled, dt_iop_roi_t *const roi_mask_scaled, 
                          const int dx, const int dy, const float opacity, const int use_sse)
{
...
static void retouch_blur(float *const in, dt_iop_roi_t *const roi_in, const int ch, 
                                    float *const mask_scaled, dt_iop_roi_t *const roi_mask_scaled, 
                                    const float opacity, 
                                    const int blur_type, const float blur_radius, dt_dev_pixelpipe_iop_t *piece, const int use_sse)
{
...
//        dt_masks_form_t *form = dt_masks_get_from_id(self->dev, formid);
            ...
            ((roi_mask_scaled.width > 2) && (roi_mask_scaled.height > 2)))
        {
          ...
          {
            retouch_clone(layer, roi_layer, wt_p->ch, mask_scaled, &roi_mask_scaled, dx, dy,
                form_opacity, wt_p->use_sse);
          }
          else if (algo == dt_iop_retouch_heal)
          {
            retouch_heal(layer, roi_layer, wt_p->ch, mask_scaled, &roi_mask_scaled, dx, dy,
                form_opacity, wt_p->use_sse);
          }
          else if (algo == dt_iop_retouch_blur)
          {
            retouch_blur(layer, roi_layer, wt_p->ch, mask_scaled, &roi_mask_scaled, 
                form_opacity, p->rt_forms[index].blur_type, p->rt_forms[index].blur_radius, piece, wt_p->use_sse);
          }
          else if (algo == dt_iop_retouch_fill)
          {
            ...
            retouch_fill(layer, roi_layer, wt_p->ch, mask_scaled, &roi_mask_scaled, 
                ...
                const int use_sse)
{
  ...
  dt_iop_retouch_preview_types_t preview_type = 0;//p->preview_type;
...
cl_int rt_adjust_levels_cl(const int devid, cl_mem dev_img, const int width, const int height, const float levels[3])
{
  cl_int err = CL_SUCCESS;
    ...
    dt_iop_retouch_global_data_t *gd)
{
  ...
  err = rt_copy_image_masked_cl(devid, dev_src, dev_layer, roi_layer, 
      ...
      opacity, gd->kernel_retouch_copy_buffer_to_buffer_masked);
  if (err != CL_SUCCESS)
  {
...
static cl_int retouch_blur_cl(const int devid, cl_mem dev_layer, dt_iop_roi_t *const roi_layer, 
    cl_mem dev_mask_scaled, dt_iop_roi_t *const roi_mask_scaled, 
    const float opacity, const int blur_type, const float blur_radius, dt_dev_pixelpipe_iop_t *piece,
    dt_iop_retouch_global_data_t *gd)
{
  cl_int err = CL_SUCCESS;
  ...
  else if (blur_type == dt_iop_retouch_blur_bilateral && fabs(blur_radius) > 0.1f)
  {
    ...
    dt_bilateral_cl_t *b = dt_bilateral_init_cl(devid, roi_mask_scaled->width, roi_mask_scaled->height, sigma_s, sigma_r);
    if(b)
    {
  ...
  err = rt_copy_image_masked_cl(devid, dev_dest, dev_layer, roi_layer, 
      dev_mask_scaled, roi_mask_scaled, 
      opacity, gd->kernel_retouch_copy_image_to_buffer_masked);
  if (err != CL_SUCCESS)
  {
...
static cl_int rt_process_forms_cl(cl_mem dev_layer, dwt_params_cl_t *const wt_p, const int scale1)
{
  ...
  // if preview a single scale, just process that scale and original image
  // unless merge is activated
  if (wt_p->merge_from_scale == 0 && wt_p->return_layer > 0 && scale != wt_p->return_layer && scale != 0) return err;
  // do not process the reconstructed image
...
//        dt_masks_form_t *form = dt_masks_get_from_id(self->dev, formid);
        ...
        if ((err == CL_SUCCESS) && (dx != 0 || dy != 0 || algo == dt_iop_retouch_blur || algo == dt_iop_retouch_fill) && 
            ((roi_mask_scaled.width > 2) && (roi_mask_scaled.height > 2)))
        {
          if (algo == dt_iop_retouch_clone)
          {
          ...
          {
            err = retouch_blur_cl(devid, dev_layer, roi_layer, 
                dev_mask_scaled, &roi_mask_scaled,  
                form_opacity, p->rt_forms[index].blur_type, p->rt_forms[index].blur_radius, piece, gd);
          }
          else if (algo == dt_iop_retouch_fill)
          {
            // add a brightness to the color so it can be fine-adjusted by the user
          ...
          if (mask_display)
            rt_copy_mask_to_alpha_cl(devid, dev_layer, roi_layer, dev_mask_scaled, &roi_mask_scaled, form_opacity, gd);
