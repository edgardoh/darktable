...
static void fft(fbw_fft_t *fft_fbw, float *image_src, const int width, const int height, const float image_scale)
{
  ...
  fft_fbw->width_fft = fftw3_find_closest_factor(fft_fbw->width_fft, FFTW_FACTORS);
  fft_fbw->height_fft = fftw3_find_closest_factor(fft_fbw->height_fft, FFTW_FACTORS);
...
*/
#ifdef _OPENMP
#pragma omp parallel for default(none) shared(img_bp, img_dest) schedule(static)
#endif
  for (int i = 0; i < stride; i++)
  {
    img_dest[i] -= img_bp[i];
