...
//  const float oddness = _oddness * (1.f - cosf(2 * M_PI * _image_scale + M_PI)) / 2.f;
//  const float oddness = _oddness;
//  const float image_scale = (1.f - cosf(2 * M_PI * _image_scale + M_PI)) / 2.f;
...
/*
static void dst(float *img_src, float *img_dest, const int width, const int height)
{
  float *img_y = NULL;

  fbw_fft_t fft_fbw = {0};

  const int y_width = width;
  const int y_height = 2*(height+1);
  
  img_y = dt_alloc_align(64, y_width * y_height * sizeof(float));
  if (img_y == NULL) goto cleanup;
  memset(img_y, 0, y_width * y_height * sizeof(float));
  
  check_nan(img_src, width * height, "dst 1");
  
  for (int y = 1; y < height+1; y++)
  {
    float *d = img_y + y * y_width;
    float *s = img_src + (y - 1) * width;
  ...
  for (int y = height+2; y < y_height; y++)
  {
    float *d = img_y + y * y_width;
    const int y1 = (y - (height+2));
    float *s = img_src + (height-y1-1) * width;
  ...
  check_nan(img_y, y_width * y_height, "dst 2");
  
  fft(&fft_fbw, img_y, y_width, y_height, 1.f);
  ...
  for (int y = 1; y < height+1; y++)
  {
    fftwf_complex *s = out_src + y * w;
    float *d = img_dest + (y-1) * width;

    for (int x = 0; x < w; x++)
    {
      d[x] = d[width-x-1] = (s[x][0]) / -2.f;
    }
  }

  check_nan(img_dest, width * height, "dst 3");

  cleanup:
    
  if (img_y) dt_free_align(img_y);
  fft_free(&fft_fbw);

}

static void idst(float *img_src, float *img_dest, const int width, const int height)
{
  float *img_y = NULL;

  img_y = dt_alloc_align(64, width * height * sizeof(float));
  if (img_y == NULL) goto cleanup;
  memset(img_y, 0, width * height * sizeof(float));

  check_nan(img_src, width * height, "idst 1");
  
  dst(img_src, img_y, width, height);
  
  check_nan(img_y, width * height, "idst 2");
  
  for (int y = 0; y < height; y++)
  {
    float *d = img_dest + y * width;
    float *s = img_y + y * width;

    for (int x = 0; x < width; x++)
    {
      d[x] = 2.f / (s[x] * (float)(height+1)); 
    }
  }

  check_nan(img_dest, width * height, "idst 3");
...
static void transpose(float *img_src, const int width, const int height)
{
  float *img_y = NULL;
  ...
  const int height_s = height;
  const int width_s = width;
//  const int height_d = width;
  const int width_d = height;
  
  for (int y = 0; y < height_s; y++)
  {
    for (int x = 0; x < width_s; x++)
    {
      float *s = img_src + y * width_s + x;
      float *d = img_y + x * width_d + y;
...
//      s[x_s] = tmp; 
  ...
  memcpy(img_src, img_y, width * height * sizeof(float));
    ...
    const float beta, const float oddness, const float image_scale)
{
  ...
  const int f2_width = width-2;
  const int f2_height = height-2;
  
  img_f2 = dt_alloc_align(64, width * height * sizeof(float));
  if (img_f2 == NULL) goto cleanup;
  memset(img_f2, 0, width * height * sizeof(float));

  img_tt = dt_alloc_align(64, width * height * sizeof(float));
  if (img_tt == NULL) goto cleanup;
  memset(img_tt, 0, width * height * sizeof(float));

  for (int y = 0; y < f2_height; y++)
  {
    float *d = img_f2 + y * f2_width;
    float *s = img_src + (y+1) * width + 1;

    for (int x = 0; x < f2_width; x++)
    {
      d[x] = s[x]; 
    }
  }

  check_nan(img_f2, f2_width * f2_height, "recontruct_laplacian 1");
  
  dst(img_f2, img_tt, f2_width, f2_height);
  
  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 2");

  transpose(img_tt, f2_width, f2_height);

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 3");
  
  dst(img_tt, img_tt, f2_height, f2_width);

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 4");
  
  transpose(img_tt, f2_height, f2_width);

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 5");
  
  for (int y = 0; y < f2_height; y++)
  {
    float *d = img_tt + y * f2_width;

    for (int x = 0; x < f2_width; x++)
    {
      const float div = 1e-15 + (2.f*cosf(M_PI*((float)x)/(float)(width-1))-2.f) + (2.f*cosf(M_PI*((float)y)/(float)(height-1)) - 2.f);
      if (div != 0.f) d[x] /= div;
      else printf("div zero\n");
    }
  }

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 6");
  
  idst(img_tt, img_tt, f2_width, f2_height);

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 7");
  
  transpose(img_tt, f2_width, f2_height);

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 8");
  
  idst(img_tt, img_tt, f2_height, f2_width);

  check_nan(img_tt, f2_width * f2_height, "recontruct_laplacian 9");
  
  transpose(img_tt, f2_height, f2_width);

  for (int y = 0; y < f2_height; y++)
  {
    float *s = img_tt + y * f2_width;
    float *d = img_dest + (y+1) * width + 1;
  ...
  if (img_f2) dt_free_align(img_f2);
  if (img_tt) dt_free_align(img_tt);


}


static void conv(fbw_fft_t *fft_fbw, const int width_fft, const int height_fft, const int r)
{
    ...
    const float beta, const float oddness, const float image_scale)
{
  ...
  fbw_fft_t fft_fbw = {0};
  fbw_fft_t fft_fbw1 = {0};
  fbw_fft_t fft_fbw2 = {0};
  ...
  const int tt_width = fft_fbw.width_fft_complex;
  const int tt_height = fft_fbw.height_fft_complex;
  
  img_tt = dt_alloc_align(64, tt_width * tt_height * sizeof(float));
  if (img_tt == NULL) goto cleanup;
  memset(img_tt, 0, tt_width * tt_height * sizeof(float));

  fftwf_complex *out_src = fft_fbw.out_src;
  
  for (int y = 0; y < tt_height; y++)
  {
    fftwf_complex *s = out_src + y * fft_fbw.width_fft_complex;
    float *d = img_tt + y * tt_width;

    for (int x = 0; x < tt_width; x++)
    {
      d[x] = s[x][0];
    }
  }

  fft(&fft_fbw1, img_tt, tt_width, tt_height, image_scale);
  ...
  fft(&fft_fbw2, img_tt, tt_width, tt_height, image_scale);

  conv(&fft_fbw1, width_fft, height_fft, 1);
  conv(&fft_fbw2, width_fft, height_fft, 0);
  
  fft_fbw1.out_src[0][0] = 0.f;
  fft_fbw1.out_src[0][1] = 0.f;

  ifft(&fft_fbw1, img_tt);
  ...
  fft_fbw2.out_src[0][0] = 0.f;
  fft_fbw2.out_src[0][1] = 0.f;

  ifft(&fft_fbw2, img_tt);
